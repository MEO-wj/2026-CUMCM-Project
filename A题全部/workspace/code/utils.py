...
方法学锁定（对应 MODELING_REPORT §9.5 M1--M9）：
...
def interface_diffusivity(C, T, props):
    """界面扩散系数 D̂_{j+1/2}=A·f_bar(T)·[Φ(C_{j+1})-Φ(C_j)]/(C_{j+1}-C_j)（M3）。

    - 浓度部分：Kirchhoff 积分平均（kirchhoff_phi），保证界面通量与 ∫D dC 一致；
    - 温度部分：f(T)=e^{-arr_T/T_K} 在界面按算术平均（温度场光滑）；
    - |ΔC|<DINTERP_TOL 时改用中点值 D((C_j+C_{j+1})/2) 防 0/0（E8）。
    返回长度 M 的界面 D̂（j=0..M-1，对应 xi_half）。
    禁用调和平均（harmonic）——见 §7.2。
    """
    a = props.d_exp
    ...
    conc_factor = np.where(
        ...
        np.exp(-a / np.maximum(Cmid, P.CFLOOR)),   # 中点值 dΦ/dC=e^{-a/C}
...
def interface_conductivity(C, props):
    """界面导热系数 k̂_{j+1/2}：k(C) 在界面按算术平均（温度方程界面项）。长度 M。
...
# ============================================================
# 环境驱动 T_air(t), C_air(t)：附件1 读取 + 末值保持外推 (H3, §7.4)
# ============================================================
...
def _assemble_moisture(C_old, C_it, T_it, props, grid, one_over_R2, R_now,
    ...
    w_j dC/dt = (2/R^2)[ξ_{j+1/2}D̂_{j+1/2}ΔC/Δξ - ξ_{j-1/2}D̂_{j-1/2}ΔC/Δξ]
    ...
    # 面导：coef_face[j] = (2/R^2) ξ_{j+1/2} D̂_{j+1/2} / Δξ，j=0..M-1
...
def _assemble_temperature(T_old, C_it, T_it, props, grid, one_over_R2, R_now,
    ...
    """温度方程 theta=1 向后欧拉三对角系统，除以 (ρc_p)_j。
...
def solve_coupled(props, t_end, dt, env, *, mode="hold", R_interp=None,
    ...
    interface: "kirchhoff"(主算) / "arithmetic" / "harmonic"（仅灵敏度对照，§8.4）。
...
def _assemble_moisture_alt(C_old, C_it, T_it, props, grid, one_over_R2, R_now,
    ...
    """对照用界面平均（算术/调和），仅供 §8.4 离散决策可证伪性验证，非主算路径。
...
def build_output_matrix(hist, t_out, R_hist, *, shrinking=False,
    ...
    """由 record_full 的场历史构造输出矩阵 (n_rows × 21)。
    ...
    返回 (time_col 长度 n_rows, matrix n_rows×21)。
    """
    rows = []
    times = []
    if t_star_s is None:
...
# ============================================================
# 解析退化验证（§8.1）：常物性圆柱 Robin 扩散 Bessel 级数
# ============================================================
...
def bessel_series_solution(xi, Fo, Bi, n_roots=40):
    """无量纲 theta(xi,Fo)=Σ 2Bi/((mu_n^2+Bi^2)J0(mu_n))·J0(mu_n xi)·e^{-mu_n^2 Fo}。
...
def analytic_verify(D_const=5e-9, t_end=3600.0, dt=1.0, C0=2.55, Cair=0.05,
    ...
    """常物性圆柱 Robin 扩散：FVM 数值解 vs Bessel 解析级数（§8.1）。
...
def bdf_cross_check(props, t_end, env, C_theta_hist, t_hist, *, R0=P.R0,
    ...
    """对 P1 水分场用 BDF 独立复算，返回与 theta=1 场的 max|ΔC|（CK10，判据<1e-3）。
...
def interface_comparison(props, env, dt, *, R0=P.R0, C0=P.C0, T0=P.T0,
                         t_cap=None):
    """同一模型仅改界面平均方式求 t*：Kirchhoff（主算）/算术/调和（CK7、§8.4）。
...
def surface_cv_residual(C_new, C_old, T_new, props, env, dt, tn1, *,
    ...
    """表面控制体离散平衡残差（P1-C2 falsifiable (a)）：在收敛解上核对
      ...
      w_M(C_M^n-C_M^{n-1})/dt = (2/R^2)ξ_{M-1/2}D̂ΔC/Δξ - (2 h_m/R)(C_M-C_air)
...
def picard_residual_trace(props, env, dt, *, tn1=None, R0=P.R0, C0=P.C0,
    ...
    先推进 n_warm 步建立非平凡场，再在下一步记录每次内迭代的 max|Δ| 序列。
