% ===================================================================
% 数据图与表格的 LaTeX 引用片段（paper-figure 步骤产出）
% 用法：把需要的 float 复制到正文对应位置，或 \input 后按 \ref 交叉引用。
% 依赖：graphicx, booktabs, float（[H]）；编译器 XeLaTeX。
% 图片路径以 figures/ 为前缀，若正文与 figures/ 同级则无需修改。
% ⛔ 流程图/架构图（fig_roadmap 等）与情景示意图（fig_scene）由
%    paper-figure-html / GPTIMG 步骤产出，不在本文件内。
% ===================================================================
...
\begin{figure}[H]
  ...
  \caption{附件2 半径收缩的 PCHIP 插值曲线与收缩速率}
...
\input{figures/TABLE_data_stats.tex}
\input{figures/TABLE_properties.tex}
\input{figures/TABLE_verification.tex}
\input{figures/TABLE_sensitivity.tex}

% ===================================================================
% 技术路线图与精确几何示意（paper-figure-html 步骤产出）
% 均为单页矢量 PDF；插入宽度已按最终有效字号校核，请勿再缩小。
% ===================================================================
...
\begin{figure}[H]
  ...
  \caption{干基质量不变量与两项抵消}
  \label{fig:tikz_drymass_derive}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth,height=0.8\textheight,keepaspectratio]{figures/fig_scene.pdf}
  \caption{热风烘干传热传质情景}
  \label{fig:scene}
\end{figure}
